...
AudioPlayer.prototype.isLoading = function() {
  ...
  return (
    this.el.networkState === this.el.NETWORK_LOADING &&
...
AudioPlayer.prototype._timeUpdate = function(e) {
  var _ref;
  if (!this.isLoading()) {
    return (_ref = this.ui) != null
      ? typeof _ref.AudioPlayerTimeUpdated === 'function'
        ? _ref.AudioPlayerTimeUpdated(this.percentComplete())
        : void 0
      : void 0;
  }
};
...
const AudioPlayerUI = (function() {
  AudioPlayerUI.prototype.transitionEvents = [
    'transitionend',
    'webkitTransitionEnd',
    'MSTransitionEnd',
    'oTransitionEnd'
...
AudioPlayerUI.prototype._unbindEvents = function() {
  ...
  return (_ref3 = this.$progressContainer) != null
    ? _ref3.off('mouseup', this.seek)
    : void 0;
};
      ...
      >
        ...
        <div className="audio-player-controls">
          ...
          <span className="audio-player-button-wrappers">
            <a
              role="button"
              className="audio-player-button small icon-backward"
            />
            <a
              role="button"
              className="audio-player-button audio-player-place-pause-button icon-play"
            />
            <a
              role="button"
              className="audio-player-button small icon-forward"
            />
          </span>
        </div>
      </div>
